...
export class WorkingMemoryService {
  ...
  async fork(parent: WorkingMemoryState, childBranchId: string): Promise<WorkingMemoryState> {
    ...
    if (existing !== undefined) {
      this.#cache.set(existing.value.id, existing.value);
  ...
  async applyEpisode(
    ...
    const taskChanged =
      current.taskId !== undefined && input.taskId !== undefined && current.taskId !== input.taskId;
    if (taskChanged) {
      next = {
        ...next,
        subgoals: [],
    ...
    for (const event of input.events) {
      ...
      if (event.kind === "verification") {
        ...
        const command =
          typeof event.payload["command"] === "string" ? event.payload["command"] : "verification";
        const ref = eventRef(event);
        const outcome: WorkingOutcome = {
          id: stableHash("working-outcome:v1", next.id, event.id, status),
  ...
  async checkpoint(state: WorkingMemoryState): Promise<void> {
    await this.#persist(this.#compact(state));
  }
  ...
  async #persist(state: WorkingMemoryState): Promise<void> {
    // Keep task continuity available even when the durable checkpoint temporarily fails.
    // Callers still receive the storage error and can emit telemetry/warnings.
